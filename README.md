<div align="center">

<img src="https://github.com/truswo/Mining-Limits/blob/1.21.9/1.21.10/src/main/resources/icon.png?raw=true" width=250px alt="A minecraft block with red border around it.">

#   Mining Limits

Adds a configurable barrier to where the player can mine.

</div>

## Overview

Adds a configurable barrier to where the player can mine.

---
## Features

- **Adds a configurable border for the highest height the player can mine**
    - Let's you mine only up to a certain height (ex: allowes you to only mine up to layer 70, any layer higher than 70 would not be minable)

    - This can be turned off in settings

- **Adds a configurable border for the lowest height the player can mine**
    - Let's you mine only down to a certain height (ex: allowes you to only mine down to layer -10, any layer lower than -10 would not be minable)

    - This can be turned off in settings

- **Adds an option to only be able to mine in one chunk**
    - Allowes you to give only the position of a block that is inside of that chunk, so you won't need to calculate what is that chunk position in the world

    - The other borders would still work, so you can limit mining to a chunk and only mine down to a certain height

- **Allowes you to have a different outline color for blocks outside of limits**
    - You can choose any color you'd want for the outline, the color and opacity are completely customizable

    - _Note: This may interfere with mods that change the block's outline (ex: [Highlight](https://modrinth.com/project/Vz96hXMh)), so it also has an option to turn this off_

- **[Mod Menu](https://modrinth.com/project/mOgUt4GM) configuration support**

- **Brazilian Portuguese translations**

---
## Author

- [Lucy (truswo/wzhazy)](https://www.github.com/truswo)
- - [Twitter (@truswo)](https://twitter.com/truswo)
- - [Modrinth (@truswo)](https://modrinth.com/user/truswo)
- - [GitHub (@truswo)](https://github.com/truswo)
